...
<section class="p-4 md:p-6">
  ...
  <form action="${action}" method="post" class="bg-white p-8">
    ...
    <div class="mt-4">
      <label class="block mb-2 text-sm font-medium text-gray-900">Revisão de Denúncia</label>
      <textarea id="reviewer_comment" name="reviewer_comment" rows="4"
                class="border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 bg-gray-100 resize-none">${not empty reviewer_comment ? reviewer_comment : ''}</textarea>
      <c:if test='${not empty reviewer_commentError}'>
        <div class="flex items-center gap-1 text-red-500 text-sm mt-2">
          <!-- Ícone de erro -->
          <p>${reviewer_commentError}</p>
        </div>
      </c:if>
    </div>
