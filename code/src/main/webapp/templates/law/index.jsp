...
<section class="p-4 md:p-6">
  <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
    ...
    <a href="/law/create/" class="flex items-center justify-center px-5 py-2 text-sm text-white transition-colors duration-200 bg-blue-500 rounded-lg gap-x-2 hover:bg-blue-600 w-full md:w-auto">
      ...
      <c:forEach items="${lawList}" var="law">
        <div class="bg-white p-4 rounded-lg shadow border border-gray-200">
          <div class="flex justify-between items-start">
            <div>
              <h3 class="font-medium text-gray-800">${law.code}</h3>
              <p class="text-sm text-gray-500 mt-1">${law.date.toString()}</p>
            </div>
            <div class="flex gap-x-2">
              <a href="/law/update/${law.code}" class="text-gray-500 hover:text-blue-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
              </a>
              <a href="/law/delete/${law.code}" class="text-gray-500 hover:text-red-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
          ...
          <h4 class="font-medium text-gray-700 mt-2">${law.title}</h4>
          <p class="text-sm text-gray-500 mt-1 line-clamp-2">${law.description}</p>
        </div>
      </c:forEach>
    </c:if>
  </div>
  ...
  <div class="flex flex-col md:flex-row items-center justify-between mt-6 gap-4">
    <div class="flex-1 text-sm text-gray-500">
      Mostrando 1 a ${lawList.size()} de ${lawList.size()} resultados
    </div>
